function config = get_defaults(config)
    ...
    if ~isfield(config, 'S_init')         
        config.S_init = []; 
    else
        S_in = config.S_init;
        dims_S = size(S_in);
        if size(dims_S,2) == 3
            config.S_init = reshape(S_in,dims_S(1)*dims_S(2),[]);
            if ( (config.use_sparse_arrays) && (~issparse(config.S_init)) )
                config.S_init = sparse(double(config.S_init));
            end
        end
    end
