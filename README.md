...
- `trace_output_option`: This is an important config, without which EXTRACT will give an error. Choose 'raw' for raw traces, 'nonneg' for non-negative traces. Keep in mind that EXTRACT underestimates the positive contaminants, which is beneficial to remove cross-contamination. Thus, raw traces can have noise baseline that is more negative than the ones estimated through other methods, such as multivariate regression.
